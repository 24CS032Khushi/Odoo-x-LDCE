...
const PALETTE = ['#14554f', '#3b82f6', '#8b5cf6', '#f59e0b', '#2a9d8f', '#64748b'];
...
export const AdminDashboardPage = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Top Stat Cards Grid: Big Number + Small Label */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Users</span>
          <p className="font-display font-bold text-2xl text-abyss">
            {summary.total_users}
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <Users className="w-3 h-3 text-ocean-teal" /> Registered
          </span>
        </div>

        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Active (7d)</span>
          <p className="font-display font-bold text-2xl text-ocean-teal">
            {summary.active_users_7d}
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <Activity className="w-3 h-3 text-ocean-teal" /> Active travelers
          </span>
        </div>

        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Trips Created</span>
          <p className="font-display font-bold text-2xl text-abyss">
            {summary.total_trips}
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <Compass className="w-3 h-3 text-ocean-teal" /> Total journeys
          </span>
        </div>

        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Shared Trips</span>
          <p className="font-display font-bold text-2xl text-blue-600">
            {summary.shared_trips}
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <Share2 className="w-3 h-3 text-blue-500" /> Public links
          </span>
        </div>

        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Avg Duration</span>
          <p className="font-display font-bold text-2xl text-abyss">
            {summary.average_duration_days}d
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <Calendar className="w-3 h-3 text-ocean-teal" /> Days / trip
          </span>
        </div>

        <div className="p-4 rounded-[18px] bg-white border border-slate-200 shadow-xs space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Avg Budget</span>
          <p className="font-display font-bold text-2xl text-ocean-teal">
            ₹{summary.average_budget >= 1000 ? `${(summary.average_budget / 1000).toFixed(0)}k` : summary.average_budget}
          </p>
          <span className="text-[10px] text-slate-500 flex items-center gap-1">
            <DollarSign className="w-3 h-3 text-ocean-teal" /> Per trip avg
          </span>
        </div>
      </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={charts.popular_cities} layout="vertical" margin={{ left: 20, right: 20, top: 10, bottom: 10 }}>
                ...
                <YAxis dataKey="name" type="category" tick={{ fontSize: 11, fill: '#071019', fontWeight: 600 }} axisLine={false} tickLine={false} width={120} />
                <Tooltip
                  formatter={(val) => [`${val} stops scheduled`, 'Popularity']}
                  contentStyle={{ backgroundColor: '#071019', color: '#fff', borderRadius: '12px', border: 'none', fontSize: '12px' }}
                />
            ...
            <tbody className="divide-y divide-slate-100">
              {charts.popular_activities.map((act) => (
                <tr key={act.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="py-3 font-bold text-abyss">
                    {act.name}
                  </td>
                  ...
                  <td className="py-3 text-right font-display font-bold text-ocean-teal">
